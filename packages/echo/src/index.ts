...
export class Emitter<T extends EventHandlers = {}> {
  private static instances = new Map<string, Emitter<any>>();
  ...
  public static retrieve<T extends EventHandlers = {}>(
    key: string,
  ): Emitter<T> {
    if (!Emitter.instances.has(key)) {
      Emitter.instances.set(key, new Emitter<T>());
    }
    return Emitter.instances.get(key) as Emitter<T>;
  }
